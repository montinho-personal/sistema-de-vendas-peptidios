...
export async function POST(req: NextRequest) {
  ...
  const vendasMesAnt = vendas.filter((v) => { const d = new Date(v.data); return d >= mesAnt && d < mesAtual })
  ...
  const media3 = ult3.length > 0 ? ult3.reduce((a, b) => a + b, 0) / ult3.length : 0

  const porProduto: Record<string, number> = {}
  vendas.forEach((v) => { porProduto[v.produto] = (porProduto[v.produto] || 0) + Number(v.precoVenda) })
...
- Mês atual: R$${vendasMes.reduce((s, v) => s + Number(v.precoVenda), 0).toLocaleString('pt-BR', { maximumFractionDigits: 0 })} (${vendasMes.length} pedidos)
- Mês anterior: R$${vendasMesAnt.reduce((s, v) => s + Number(v.precoVenda), 0).toLocaleString('pt-BR', { maximumFractionDigits: 0 })}
